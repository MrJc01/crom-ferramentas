...
window.computeDiff = () => {
    ...
    // Very simple line-by-line diff for basic functionality
    // Ideally we would use a library like 'diff' (jsdiff) but we want to be zero-dep if possible or assume vendor.
    // Let's implement a naive line diff.

    const linesOrig = original.split('\n');
    const linesMod = modified.split('\n');

    let html = '';
    const maxLines = Math.max(linesOrig.length, linesMod.length);

    for (let i = 0; i < maxLines; i++) {
        const lineO = linesOrig[i];
        const lineM = linesMod[i];

        if (lineO === lineM) {
            html += `<div class="text-slate-500 py-0.5 px-2 hover:bg-slate-50 dark:hover:bg-slate-800 flex"><span class="w-8 text-xs select-none opacity-50 text-right mr-4 border-r pr-2">${i + 1}</span> <span>${escapeHtml(lineO || '')}</span></div>`;
        } else if (lineO !== undefined && lineM !== undefined) {
            // Changed line (naive)
            html += `<div class="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 py-0.5 px-2 flex"><span class="w-8 text-xs select-none opacity-50 text-right mr-4 border-r pr-2">- ${i + 1}</span> <span>${escapeHtml(lineO)}</span></div>`;
            html += `<div class="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 py-0.5 px-2 flex"><span class="w-8 text-xs select-none opacity-50 text-right mr-4 border-r pr-2">+ ${i + 1}</span> <span>${escapeHtml(lineM)}</span></div>`;
        } else if (lineO !== undefined) {
            // Deleted
            html += `<div class="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 py-0.5 px-2 flex"><span class="w-8 text-xs select-none opacity-50 text-right mr-4 border-r pr-2">- ${i + 1}</span> <span>${escapeHtml(lineO)}</span></div>`;
