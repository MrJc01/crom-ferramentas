    ...
    render: () => `
        <div class="flex flex-col items-center gap-6 py-10">
            <input type="text" id="qrText" placeholder="Digite URL ou texto..." 
                ...
                onkeyup="CromModules.QR.generate(this.value)">
            
            <div id="qrCanvasContainer" class="p-4 bg-white rounded-xl shadow-lg border border-slate-100 hidden">
                <div id="qrcode"></div>
            </div>
            
            <button onclick="CromModules.QR.download()" id="qrDownloadBtn" class="hidden text-sm text-indigo-500 hover:underline">Baixar Imagem</button>
        </div>
    `,
    init: () => {
        window.CromModules = window.CromModules || {};
        window.CromModules.QR = {
            generate: (text) => {
