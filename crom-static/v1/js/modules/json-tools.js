window.CromApp.registerTool({
    ...
    render: () => `
        ...
        <div class="mt-4 flex gap-2 justify-end">
            <button onclick="window.processJson('minify')" class="px-4 py-2 bg-slate-200 dark:bg-slate-800 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-700 font-bold text-sm">Minificar</button>
            <button onclick="window.processJson('format')" class="px-6 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 font-bold text-sm">Formatar (Prettify)</button>
        </div>
    `
});

// Helper Functions Globally Available
window.processJson = (action) => {
